...
export async function createFatoraCheckoutSession(params: FatoraCheckoutParams): Promise<string> {
  ...
  try {
    // Format the request to EXACTLY match the working curl command
    const payload = {
      amount: params.amount,
      currency: "USD",
      order_id: params.orderId,
      client: {
        name: "User", // Use "User" as the name - this works in the curl command
        phone: params.customerPhone,
        email: params.customerEmail,
        // No additional client fields
      },
      language: "en",
    ...
    console.log("EXACT PAYLOAD BEING SENT TO FATORA:", JSON.stringify(payload, null, 2))
