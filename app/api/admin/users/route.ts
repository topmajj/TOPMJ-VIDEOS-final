...
export async function GET(request: Request) {
  try {
    ...
    const authUsersQuery = supabaseAdmin.auth.admin.listUsers({
      page: page,
      perPage: pageSize,
    })

    const { data: authData, error: authError } = await authUsersQuery
    ...
    if (filter === "admins") {
      filteredUsers = filteredUsers.filter((user) => user.profile?.is_admin)
    } else if (filter === "recent") {
      const thirtyDaysAgo = new Date()
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30)
      filteredUsers = filteredUsers.filter((user) => new Date(user.created_at) >= thirtyDaysAgo)
